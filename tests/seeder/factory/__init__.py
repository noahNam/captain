...
from app.extensions.utils.time_helper import get_jwt_access_expired_timestamp, get_jwt_refresh_expired_timestamp, \
    get_server_timestamp, get_jwt_access_expired_time_delta, get_jwt_refresh_expired_time_delta
...
class InvalidJwtFactory(BaseFactory):
    ...
    refresh_token = make_custom_jwt(user_id,
                                    ...
                                    now=datetime.now() - timedelta(days=1),
                                    delta=get_jwt_refresh_expired_time_delta())
    access_expired_at = datetime.now() - timedelta(days=1) + get_jwt_access_expired_time_delta()
    refresh_expired_at = datetime.now() - timedelta(days=1) + get_jwt_refresh_expired_time_delta()
